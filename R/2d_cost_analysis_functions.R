...
#' @param freq_taken frequency of medicaiton
#' @param basis_time basis for time
#' @param unit_cost_data  unit costs data
#' assumes unit cost data contains the columns, name of medication, form/type, strength,
...
#' @param equiv_dose if cost per equivalent doses are to be calcualted, provide equiv_dose
...
#' res=microcosting_patches(ind_part_data,"Drug", "patch_strength", "patch_dose_unit","patch_no_taken",
...
microcosting_patches <- function(ind_part_data,
  ...
  if (is.null(unit_cost_data) | is.null(unit_cost_column) | is.null(cost_calculated_in)) {
    stop("Error - Provide unit costs data")
  }
  ...
  if (IPDFileCheck::check_column_exists(cost_calculated_in,unit_cost_data) != 0)
    stop("Error - basis of unit cost calculation should exist in the unit cost data")
  ...
  for (i in 1:nrow(ind_part_data)) {
    ...
    if (!is.null(dose_medication)  & !is.na(dose_medication)) {
      ...
      total_med_basis = how_many_taken * freq_mutliplier
...
#' @param list_of_code_freq if frequency is coded, give the code:frequency pairs
#' @param list_of_code_dose_unit if unit is coded, give the code:unit pairs
#' @param equiv_dose if cost per equivalent doses are to be calcualtes, provide equiv_dose
#' @return the calculated cost of tablets along with original data
#' @examples
#' freq_desc <- c("Once a day","Twice a day", "Three times a day",
...
#' ind_part_data = load_trial_data(data_file)
#' med_costs = load_trial_data(med_costs_file)
#' res = microcosting_tablets(ind_part_data, "Name", "tab_dosage", "tab_dosage_unit","tab_no_taken",
...
#' @export
microcosting_tablets <- function(ind_part_data,
                                 name_med, dose_med, dose_unit,
                                 no_taken, freq_taken, basis_time,
                                 unit_cost_data, unit_cost_column,
                                 cost_calculated_in, strength_expressed_in,
  ...
  ind_part_data[["tot_basis_tablets"]] = unlist(list_total_med_basis)
  ind_part_data[["totcost_tablets_basis"]] = unlist(list_total_cost_basis)
  ind_part_data[["totcost_tablets_basis_equiv_dose"]] = unlist(list_total_cost_basis_equiv_dose)
  ind_part_data[["totcost_timeperiod_tablets"]] = unlist(list_total_cost_timeperiod)
  ind_part_data[["totcost_tablets_timeperiod_equiv_dose"]] = unlist(list_total_cost_timeperiod_equiv_dose)
  return(ind_part_data)
}
##########################################################################################################
...
#' @param bottle_remain frequency of medicaiton
#' @param unit_cost_data  unit costs data
#' #' assumes unit cost data contains the columns, name of medication, form/type, strength,
#' unit of strength, preparation, unitcost, size and size unit
#' (in which except name and form are to be passed in)
...
#' ind_part_data = load_trial_data(data_file)
#' med_costs = load_trial_data(med_costs_file)
#' res = microcosting_liquids(ind_part_data, "Drug", "liq_dosage", "liquid_dose_unit",
...
microcosting_liquids <- function(ind_part_data,
                                 name_med, dose_med, dose_unit,
                                 bottle_size, bottle_remain,bottle_size_unit = NULL,
                                 unit_cost_data, unit_cost_column,
  ...
  form_pattern = c("form","drug form", "patch/tablet/liquid/injection", "type")
  bool_res = unlist(lapply(form_pattern, IPDFileCheck::check_colno_pattern_colname,colnames(unit_cost_data)))
  if (any(bool_res))
    form_ind = which( bool_res == TRUE)
  else
  ...
  if (!is.null(list_of_code_bottle_size)) {
    size_and_code <- stats::setNames(as.list(list_of_code_bottle_size[[1]]),list_of_code_bottle_size[[2]])
  ...
  for (i in 1:nrow(ind_part_data)) {
    ...
    if (!is.null(dose_medication)  & !is.na(dose_medication)) {
      ...
      subset1 = unit_cost_data[toupper(unit_cost_data[[name_col_no]]) == toupper(name_medication),]
      subset2 = subset1[subset1[form_col_no] == "Liquid" | subset1[form_col_no] == "Liquids",]
      unit_used_costing = unique(subset2[[unit_col_no]])
      if (length(unit_used_costing) != 1)
        stop("unit used for costing tablets is not unique !!!")
    ...
    list_total_bottle_timeperiod <- append(list_total_bottle_timeperiod,no_bottle_timeperiod)
    list_total_bottle_timeperiod_equiv_dose <- append(list_total_bottle_timeperiod_equiv_dose,no_bottle_timeperiod_equiv_dose)
    list_total_cost_timeperiod <- append(list_total_cost_timeperiod, total_cost_timeperiod)
    list_total_cost_timeperiod_equiv_dose <- append(list_total_cost_timeperiod_equiv_dose,total_cost_timeperiod_equiv_dose)
  }
...
#' @param unit_length_use the column name that contains how many or how long used
#' @param unit_cost_data  unit costs data where the assumption is that the unit cost for resources
#' such as hospital use, gp visit are listed in column resource/reource use with unit costs in another
#' coulmn and the units calculated as in another column
#' @param name_use_unit_cost name of resource use (the column name in the unit cost
#' data is assumed to be name/resource/type etc) in unit cost data
#' @param unit_cost_column column name of unit cost in unit_cost_data
#' @param cost_calculated_in column name of unit where the cost is calculated
#' @param list_code_use_indicator if the column name_use_col  shows codes to indicate
#' the resource use provide the list of codes and resource use
#' for eg., list(c("yes", "no", c(1,2)))
#' @param list_code_provider_indicator  if the column each_use_provider_indicator
#' shows codes to indicate the resource use
#' provide the list of codes and resource use for eg., list(c("yes", "no", c(1,2)))
...
#' res = costing_resource_use(ind_part_data[1,],
...
costing_resource_use <- function(ind_part_data,
  ...
  if (is.null(list_code_use_indicator)) {
    ...
    use_code <- stats::setNames(as.list(list_code_use_indicator[[1]]),list_code_use_indicator[[2]])
    use_from_data = ind_part_data[[name_use_col]]
    use_ind_from_code = toupper(use_code[use_from_data])
  }

  #check length of each_length_use, each_number_use, each_use_provider_indicator to be same
  ...
  if (any(bool_res))
    ...
    stop("Error- Name of the resource use is not found. Please use name, resource, resource use,
  ...
  for (i in 1:nrow(ind_part_data)) {
    if(!is.null(name_use_unit_cost)){
      subset1 = unit_cost_data[toupper(unit_cost_data[[name_col_no]]) == toupper(name_use_unit_cost),]
    }else{
    ...
    if(!is.null(name_use_unit_cost))
      ...
      ind = 1
    if(!is.na(ind) & ind > 0) {
      if (IPDFileCheck::check_column_exists(unit_length_use, ind_part_data) != 0 )
        uni_expr = unit_length_use
      else
        uni_expr = ind_part_data[[unit_length_use]][i]
      total_length_num = 0
      for(m in 1:length(each_length_num_use)){
        if (IPDFileCheck::check_column_exists(each_length_num_use[m], ind_part_data) == 0){
          if (IPDFileCheck::check_column_exists(each_use_provider_indicator[m], ind_part_data) == 0){
            if (is.null(list_code_provider_indicator)) {
              ...
              use_code <- stats::setNames(as.list(list_code_provider_indicator[[1]]),list_code_provider_indicator[[2]])
              this_column_name = unlist(each_use_provider_indicator[m])
            ...
            if (!is.null(list_code_provider_indicator)) {
              stop("Error - column do not exist and given codes for provider indicator")
            }else{
