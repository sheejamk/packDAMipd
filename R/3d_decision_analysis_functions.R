...
check_list_markov_models <- function(list_markov) {
  ...
  for (i in 1:no_comparison) {
    names_strategies <- append(names_strategies, list_markov[, "strategy"][[i]]$name_strategy)
  }
  ...
  for (i in seq_len(no_comparison)) {
    no_states_all <- append(no_states_all, list_markov[, "strategy"][[i]]$transition_matrix$no_states)
...
plot_ceac <- function(list_markov, threshold_values, comparator, currency = "GBP") {
  if (is.null(threshold_values)) stop("Error - Threshold values can not be null")
  if (check_list_markov_models(list_markov) == 0) {
