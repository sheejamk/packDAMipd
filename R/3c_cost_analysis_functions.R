...
microcosting_tablets_patches <- function(form,ind_part_data,
  ...
  if (any(bool_res)) {
    ...
    stop("Error- Form of the medication is not found. Please use form, drug form, patch/tablet, or type to indicate
...
microcosting_liquids <- function(ind_part_data,
  ...
  if (is.null(unlist(size_from_code))) {
    stop("Error - size_from_code can not be null - check the input for list of size and codes")
  }
...
#' such as hospital use, gp visit are listed in column resource/resource use with unit costs in another
...
costing_resource_use <- function(ind_part_data,
  ...
  # if the name of the resource use in unit cost is not null, find the subset of data with that resource use
  # or get the column name with the resource use given at the ith row of the ind data (these might be
  # the resource use where it is not needed to specify the use by a yes/no and rather by number of contacts)
  for (i in 1:nrow(ind_part_data)) {
    ...
    if (!is.na(ind) & ind > 0) {
      ...
      total_length_num <- 0
      # each_length_num_use is the column names where it is given the length of use
      for (m in 1:length(each_length_num_use)) {
        ...
        if (IPDFileCheck::check_column_exists(each_length_num_use[m], ind_part_data) == 0) {
          ...
          if (IPDFileCheck::check_column_exists(each_use_provider_indicator[m],
                                                ind_part_data) == 0) {
            # if the use corresponding to the provider is coded, then use the code
