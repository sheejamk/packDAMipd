...
test_that("testing adding EQ5D5L values to the data", {
  ...
  results <- value_eq5d5L_IPD(trialdata, NA)
  ...
  results <- value_eq5d5L_IPD(trialdata, 99)
...
test_that("testing adding EQ5D3L values to the data", {
  ...
  results <- value_eq5d3L_IPD(trialdata, NA)
  ...
  expect_error(value_eq5d3L_IPD(trialdata, NA))
  # Error - data should not be NULL
  ...
  results <- value_eq5d3L_IPD(trialdata, 99)
...
test_that("testing adding EQ5D5L values to the data", {
  ...
  library(valueEQ5D)
  results <- map_eq5d5Lto3L_VanHout(trialdata, NA)
  ...
  expect_error(map_eq5d5Lto3L_VanHout(trialdata, NA))
  #Error - data should not be NULL
  ...
  results <- map_eq5d5Lto3L_VanHout(trialdata, -99)
...
test_that("testing conversion of ADL responses to scores", {
  ...
  results <- value_ADL_scores_IPD(trialdata, c("tpi"), NA, adl_scoring.df)
  ...
  results <- value_ADL_scores_IPD(trialdata, c("tpi"), 99, adl_scoring.df)
  ...
  expect_error(value_ADL_scores_IPD(trialdata, NA, NA))
  #Error data should not be NULL
  expect_error(value_ADL_scores_IPD(NULL, c("tpi"), NA, adl_scoring.df))

  #Error no matching columns
  expect_error(value_ADL_scores_IPD(trialdata, NULL, NA, adl_scoring.df))

  trialdatafile <- system.file("extdata", "trial_data_sampleEq5d.csv",
                               package = "packDAMipd")
  trialdata <- read.csv(trialdatafile)

  results <- value_ADL_scores_IPD(trialdata, c("tpi"), NA, NULL)
  ...
  expect_error(value_ADL_scores_IPD(trialdata, c("tpi"), NA, adl_scoring.df))
  ...
  results <- value_ADL_scores_IPD(trialdata, c("tpi"), -99, NULL)
...
test_that("testing adding EQ5D5L values to the data", {
  ...
  results <- value_Shows_IPD(trialdata, c("shows"), NA)
  ...
  expect_error(value_Shows_IPD(trialdata, NA, NA))
  #Error data should not be NULL
  ...
  trialdatafile <- system.file("extdata", "trial_data_sampleEq5d.csv",
                               package = "packDAMipd")
  trialdata <- read.csv(trialdatafile)
  results <- value_Shows_IPD(trialdata, c("qsy"), NA)
  ...
  expect_error(value_Shows_IPD(trialdata, c("qsy"), NA))
  ...
  results <- value_Shows_IPD(trialdata, c("shows"), -99)
...
test_that("testing conversion of promis3a responses to scores", {
  ...
  library(valueEQ5D)
  results <- value_promis3a_scores_IPD(trialdata, c("tpi"), NA, NULL)
  ...
  expect_error(value_promis3a_scores_IPD(trialdata, NA, NA))
  #Error data should not be NULL
  expect_error(value_promis3a_scores_IPD(NULL, c("tpi"), NA, NULL))

  #Error no matching columns
  expect_error(value_promis3a_scores_IPD(trialdata, NULL, NA, NULL))
  ...
  results <- value_promis3a_scores_IPD(trialdata, c("tpi"), 99, NULL)
  expect_equal(results$promis3aTscore, c(36.3, 51.4, 99))

  trialdata <- data.frame(
    "tpi.q1" = c(1, 2), "tpi.q2" = c(1, 2), "tpi.q3" = c(1, 9)
  )
  expect_error(value_promis3a_scores_IPD(trialdata, c("tpi"), NA, NULL))
  trialdata <- data.frame(
