...
test_that("testing defining parameter for probabilistic sensitivity analysis",  {
  param_list<-define_parameters(cost_direct_med_B = 1774,
...
test_that("testing doing probabilistic sensitivity analysis",  {
  ...
  tm <- transition_matrix(3, tmat, c("tpAtoA", "tpAtoB", "tpAtoC",
  ...
  expect_error(do_psa(mono_markov, define_parameters_psa(param_list, sample_list, sample_list)))
  expect_warning(do_psa(mono_markov, param_table, -1))
  expect_error(do_psa(mono_strategy, param_table))
})
###############################################################################
...
test_that("testing listing probabilistic sensitivity analysis results parameterwise",  {
  ...
  comb_markov <-markov_model(comb_strategy, 20, c(1, 0, 0), c(0, 0, 0), c(0, 0, 0),
                             discount =c(0.06, 0), param_list)
