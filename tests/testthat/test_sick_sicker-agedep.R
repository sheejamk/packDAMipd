...
tmat_cost <- rbind(c(NA, 1,NA,2), c(NA, NA,NA,3),c(NA, NA, NA,4), c(NA,NA,NA,NA))
colnames(tmat_cost) <- rownames(tmat_cost) <- c("H","S1" ,"S2","D")
tmat_util <- rbind(c(NA, 1,NA,NA), c(NA, NA,NA,NA),c(NA, NA, NA,NA), c(NA,NA,NA,NA))
...
a <- define_parameters(age_init = 25,
                      mortality_age = "get_mortality_from_file(mortality_file, age_init -1 + markov_cycle)",
                      p_HDage = "1-exp(-mortality_age)", p_HS1  = 0.15, p_S1H = 0.5,
...
uc_markov <- markov_model(uc_strategy, 85, c(1, 0,0,0),c(2000,0,0,0),c(1,0,0,0),discount = c(0.03,0.03),a)
uc_markov$cost_matrix
...
D <- health_state("D", cost = "c_D ",utility = "u_D")
health_states <- combine_state(H,S1,S2,D)
trt_strategy <- strategy(tm, health_states, "treatment",tm_cost,tm_util)

trt_markov <- markov_model(trt_strategy, 85, c(1, 0,0,0),c(2000,0,0,0),c(1,0,0,0),discount = c(0.03,0.03),a)
trt_markov$utility_matrix

plot_model(trt_markov)
trt_markov$cost_matrix
list_markov <- combine_markov(uc_markov, trt_markov)
calculate_icer_nmb(list_markov, threshold = 20000, comparator = "Usual care" )

plot_ceac(list_markov,threshold_values = c(1000,2000,5000,7000,10000,15000,20000), comparator = "Usual care" , currency = "GBP")
