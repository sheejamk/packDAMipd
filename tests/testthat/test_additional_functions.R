...
test_that("testing getting parameters between operators", {
  ...
  expect_equal(find_param_from_def_prob_distrbn("gamma(mean = sqrt(2), b =17)"), c("mean", "b"))
  expect_error(find_param_from_def_prob_distrbn("gamma(shape, location)"))
})
###############################################################################
...
context("testing  getting required parameters to define probabilty distribution")
test_that("testing  getting required parameters to define probabilty distribution", {
  expect_equal(check_estimating_required_params(
    c("mean", "sd"), c("shape", "rate"),
    "gamma(mean = 10 ,sd=1)", "gamma"
  ), c(100, 10), tolerance = 1e-4)
  expect_equal(check_estimating_required_params(
    c("mean", "sd"), find_required_parameter_combs("weibull"),
    "weibull(mean = 10 ,sd=1)", "weibull"
  ), 0)
  expect_equal(check_estimating_required_params(
    c("mean", "sd"), c("shape", "rate"),
    "gamma(mean = sqrt(10) ,sd=1)", "gamma"
  ),
  c(10.000000, 3.162278),
  tolerance = 1e-4
  )
  expect_error(check_estimating_required_params(
    c("mean", "sd"), find_required_parameter_combs("weibull"),
