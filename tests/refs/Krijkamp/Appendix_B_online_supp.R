...
# Makes use of:
  ...
  for (i in 1:n.i) {
    ...
    m.E[i, 1] <- Effs(m.M[i, 1], dur, Trt, X = X[i])  # estimate QALYs per individual of the initial health state conditional on treatment, duration of being sick/sicker and individual characteristics
  ...
  if (TS.out == TRUE) {  # create a  matrix of transitions across states
    ...
    colnames(TS) <- paste("Ind",   1:n.s, sep = " ")    # name the  of the matrix
  } else {
    TS <- NULL
  }
  ...
  if (TR.out == TRUE) {  # create a trace from the individual trajectories
    ...
    rownames(TR) <- paste("Cycle", 0:n.t, sep = " ")  # name the columns of the matrix
  } else {
    TR <- NULL
  }
